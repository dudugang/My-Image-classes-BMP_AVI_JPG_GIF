/***
	...
	CBaseImage 는 그래픽스의 기본 클래스
	이 클래스의 파생클래스로부터 JPG , BMP 를 다룬다..
...
BOOL CBaseImage::Create(SIZE size, int colornum,COLORREF backcolor)
{
	...
	if(colornum)
		...
		if(m_pDib)
			delete m_pDib;
		m_pDib= new char [pBitmapInfo->bmiHeader.biSizeImage];	// Error... Because of bmiHeader.biSizeImage has many data(4032213064).. ㅡ0ㅡ;;
...
/*
BOOL CBaseImage::LoadJpg(CString strFile)
{
	return	TRUE;
}
*/